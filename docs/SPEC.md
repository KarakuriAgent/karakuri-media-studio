...
- **`veo3_1_fast` / `veo3_1_quality`（family `veo`、backend `kie`）** はテンプレートを持たない**外部 API**の
  ...
  なお `full`（画像→動画）は **1 ジョブ = 1 バックエンド**（§5.2）なので画像ステージも kie の
  ワークフローである必要がある。kie の画像ワークフローを足すまでは、実際には `i2v` /
  プロンプトだけの生成とラストフレーム連鎖で使う（ComfyUI の画像ワークフローと組んだ `full` は 422）
