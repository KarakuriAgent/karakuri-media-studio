...
LLM を回す CLI は設定 `agent_cli`（既定 `grok`）で選ぶ。**プロンプト作成チャット（§4.3）とヘルスチェック（`--version`）が同じ選択に従う**（アプリ内で LLM を呼ぶのはこの 2 つだけ。**英訳・タグ付け・NSFW 判定はアプリ内では行わない**: 英訳は外部エージェント＝ SKILL 経由の `/api/v1` の仕事で、制作そのものを回すのも同じく外部エージェント）。
...
ファイルストアの 3 つは既定ではリポジトリ直下だが、環境変数 `KARAKURI_DATA_DIR` を
書くとその下（NAS など、リポジトリの外）に置ける（`paths.DATA_DIR`）。`app.db` と
`runtime/` は対象外でリポジトリ固定（SQLite を CIFS/NFS に置くとロックが壊れるため）。
DB に残る旧プレフィックスの絶対パスは読み出しのたびに `paths.rebase_stored_path` が
いまの置き場（`outputs` / `assets` / `library` は `DATA_DIR`、`runtime` は `ROOT`）へ
載せ替えるので、置き場を移しても履歴はリンク切れにならない。
...
- **タグ・表示名は付けた人のもの**: アプリ内で LLM を呼ばない（§4.1）ので、タグと表示名の自動生成は
  行わない。既定の表示名は英語プロンプトの先頭なので、探しやすくしたいときは登録時に `name` /
  `tags` を書くか、あとから `PATCH /api/library/{id}` で直す（外部エージェントからもできる）。
  素材が増減・変更されたら WS（`type: "library"`）で画面に伝え、開いているライブラリモーダルは
  一覧を読み直す
...
- **フレーム精度**: 量子化するのは**クリップの尺ではなくタイムライン上の境界**
  （`frame_count` = `round(t * fps)`、並び全体は `frame_plan`）。クリップの枚数は
  `round(終わり * fps) - round(始まり * fps)` で、隣り合うクリップは境界を共有するので
  丸め誤差が累積しない（尺をひとつずつ丸めていた頃は 51 クリップで 4 フレーム不足した）。
  総枚数は必ず `round(全長 * fps)` になる。素材の切り出し位置（`in_ms`）も同じ規則で
  フレーム番号にする。秒のまま切ると端数フレームが捨てられて連結後に最大 2 フレーム
  先走るので、各クリップは 1 フレームぶん余分に取ってから `trim=end_frame=<枚数>` で
  ちょうどその枚数に切り、音も `apad` + `atrim` で同じ長さへ揃える。繋ぎの重なりも
  同じ境界から出し（`overlap = 前の終わり - 自分の始まり`）、出力そのものも `-frames:v` で締める
- **1 フレームに満たないクリップは落とす**（`build_command`）。自動配置や差し込みが作る
  数ミリ秒の隙間がこれで、`concat` に尺 0 のセグメントを渡すと ffmpeg が
  `Error while filtering: Invalid argument` で落ちる。境界基準なので落としても全長は
  変わらない（そのぶんは隣のクリップが吸う）
...
- **焼き上がりの検算**: `ffprobe -count_frames` で総フレーム数を数え、計画
  （`ExportSpec.total_frames` = `round(全長 * fps)`）と違えば、その差を `warnings` と
  ログに出す（`フレーム数が計画と違います（計画 4644f / 実測 4640f / 差 -4f）`）。結果は `timeline_exports` の
...
- **投入時の NSFW 指定**: [実行] の直上に「🫣 NSFW として投入（オフなら生成後に自動判定）」のチェックボックスを置く。**チェックしたときだけ** `JobCreate.nsfw = true`（manual 扱いで自動判定をスキップ）を送り、オフのままなら何も送らず従来どおり生成後の自動判定に任せる（既定オフ）。自動判定は**プロンプトのキーワードだけ**で決める（LLM は呼ばない、§4.1 / `app/nsfw.py`）ので取りこぼしはあり、人がトグルで直せば `manual` になって以後は触られない
...
- **ファイルタブ**（`components/library/LibraryView.tsx`、§7.2）: **このアプリが持っている
  ...
  **作品**（すべての作品 / `GET /api/studio/projects` の一覧）と
  ...
  - WS の `type: "library"`（ライブラリの更新通知）とジョブ一覧の更新を受け取ると、
    その出どころを読み直す
  - **構図リファレンス動画（ブロッキング、§7.2）**は上部バーの [構図リファレンスを作る] から
    ...
    - 初期シーンは 16:9 / 4 秒 / カメラ `(0, 1.6, 6)` → 注視点 `(0, 1.0, 0)` / fov 47 で、
      ...
      モーダルの中のバナーに出す
  - ライブラリの更新通知（WS の `type: "library"`）が来たら一覧を読み直す
...
WS   /api/ws                     … 進捗と更新の配信。種別は `type: "job"`（ジョブの状態と進捗）/ `"chat"`（プロンプト作成チャットのターン）/ `"library"`（ライブラリの更新通知）/ `"model_download"`（不足モデルの取得）/ `"timeline_export"`（書き出し）/ **`"studio"`**（外部 API による脚本・素材の更新。`project_id` / `entity` / `id` / `op` だけを流し、正本は DB）/ **`"form"`**（生成フォームの下書き。`revision` / `updated_by` / `values`、§7.5）/ **`"ui"`**（`op: "navigate"` で画面を切り替えさせる）。配信は `app/ws.py` の `publish*` 関数で、**どれも例外を投げない**（通知の失敗で生成や編集を壊さない）
...
backend/            FastAPI アプリ
  ...
  app/nsfw.py       ジョブ / セッションの NSFW 判定（キーワードのみ。LLM は使わない）
...
15. **内蔵エージェントモードとキャンバスは撤去**。制作を回すのは外から `/api/v1` を叩く
    ...
    アプリ内に残る LLM 用途は**プロンプト作成チャットとヘルスチェック**の 2 つだけ。
    **英訳・タグ付け・NSFW 判定はアプリ内では行わない**（英訳は外部エージェントの仕事。
    日本語のまま生成しようとすると断られ、`english_prompt` に英語版を保存してから回す）（§4.1）
