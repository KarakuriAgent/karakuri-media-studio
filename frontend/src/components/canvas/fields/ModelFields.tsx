...
export default function ModelFields({
  ...
  const videoLoras = lorasForTarget(options?.loras ?? [], 'video')
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      <WorkflowPicker
        workflows={workflows}
        value={data.workflow}
        onChange={(workflowId) => patch({ workflow: workflowId })}
        modelLabel="モデル"
        modeLabel="モード"
      ...
      {(supportsSpeedup(workflow, SAGE_ATTENTION) ||
        supportsSpeedup(workflow, EASY_CACHE)) && (
        <Field label="高速化" hint="未チェックはサーバー設定の既定値に従う">
            ...
            {supportsSpeedup(workflow, SAGE_ATTENTION) && (
              <label className="flex cursor-pointer items-center gap-1.5">
                <input
                  type="checkbox"
                  className="h-3 w-3 accent-accent-500"
                  checked={data.params.sage_attention === true}
                  onChange={(event) =>
                    patchParams({
                      sage_attention: event.target.checked ? true : null,
                    })
                  }
                />
                Sage Attention
            ...
            {supportsSpeedup(workflow, EASY_CACHE) && (
              <label className="flex cursor-pointer items-center gap-1.5">
                <input
                  type="checkbox"
                  className="h-3 w-3 accent-accent-500"
                  checked={data.params.easy_cache === true}
                  onChange={(event) =>
                    patchParams({ easy_cache: event.target.checked ? true : null })
                  }
                />
                EasyCache
              </label>
