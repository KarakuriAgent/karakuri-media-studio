...
| 画像+音声→ダンス動画 (Wan Dancer) | 開始フレーム / 音声 | **Wan 2.2 系**。渡した曲に合わせて踊る動画を生成する（曲がそのまま音声トラックになる）。プロンプトは書かず、「踊りの種類」「動きの大きさ」を選ぶだけ。尺は音声の長さに自動で合わせる（5〜30 秒） |

- 既定は **ID-LoRA**（リップシンク）です
- 「画像＋動画」モードのプルダウンには**開始フレームを受け取れるワークフローだけ**が並びます
- ワークフローによっては、プロンプトの代わりに**選択式のプルダウン**が出ます（Wan Dancer の
  「踊りの種類」「動きの大きさ」「尺」）。尺のように「自動」を選べる項目は、未指定なら入力
  （この場合は音声の長さ）から決まります。動画プロンプトは任意で、書けばテンプレート文を置き換えます
- LoRA を挿せないワークフロー（Wan Dancer）では LoRA（動画）欄そのものが出ません
...
渡るため、**画像 4 種・動画 8 種・音声 2 種すべてのワークフロー**を使い分けて計画できます
（`continue` でのワークフロー切替も可。ただし開始フレームを受け取れるワークフローのみ）。
ワークフローが選択式フィールドを持つ場合（Wan Dancer）は、その選択肢もカタログに載るので
`selects` で指定できます（尺のように省略すれば自動で決まる項目もあります）。
