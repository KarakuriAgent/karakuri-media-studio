...
# jobs do not embed them (see build_system_prompt).
...
def _required_fields(mode: str, workflow_id: str) -> list[str]:
    ...
    :func:`app.models.missing_job_fields` is what ``POST /api/jobs`` enforces,
    so asking it with an *empty* job yields exactly the fields that are mandatory — the prompt rules can never
