...
参照入力に「複数のパネルを並べた 1 枚の画像」を取るワークフロー向けのシートを
作る。作法はこう:
...
* **大きく写っているパネルほど精密に再現される**ので、主役（キャラクター）は
  大きく、脇役（小物・背景）は小さく置く
* シートのアスペクト比は出力動画に合わせるのが望ましい（比が合っていれば
  ワークフロー側のパディングで余白が出ない）
...
def _contain(image: Image.Image, panel: Panel) -> tuple[Image.Image, int, int]:
    ...
    素材を引き伸ばすと粗くなるが、パネルを大きく使うほうがモデルには効くので
