...
from app import backends, codex_media, config, grok_media, kie  # noqa: E402
...
def isolated_backends(monkeypatch, tmp_path_factory):
    ...
    monkeypatch.setattr(
        codex_media, "auth_path", lambda: Path("/nonexistent/.codex/auth.json")
    )
    backends.invalidate()
    yield
